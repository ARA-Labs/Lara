...
prop_frozenNumbers = once $ ioProperty $ do
  ...
  pure $
    conjoin
      ...
      , counterexample "load-bearing total" (csLoadBearingTotal rep === 36)
      , counterexample "load-bearing provenance" (csLoadBearingProvenance rep === [(AiExecuted, 36)])
      , counterexample "load-bearing kind" (csLoadBearingKind rep === [(Attested, 20), (Observed, 16)])
      , counterexample "load-bearing paper-anchored" (csLoadBearingPaperAnchored rep === 13)
      , counterexample "all leaves total" (csAllTotal rep === 167)
      , counterexample "all leaves provenance" (csAllProvenance rep === [(AiExecuted, 167)])
      , counterexample "all leaves kind" (csAllKind rep === [(Attested, 106), (Observed, 61)])
      , counterexample "all leaves paper-anchored" (csAllPaperAnchored rep === 49)
      , counterexample "typed attacks" (csTypedAttacks rep === 3)
      , counterexample "dead-end attacks" (csDeadEndAttacks rep === 2)
      , counterexample "strict steps" (csStrictSteps rep === 0)
      , counterexample "strict certified" (csStrictCertified rep === 0)
