...
`type(scope): subject` in the imperative, lowercase subject, no trailing period —
e.g. `feat(wire): bound S-expression nesting` or `theory(m5): relate expanded
prose under renaming`. Use the narrowest sensible scope (`fix(cli)`, `docs(ara)`,
bare `docs:` when repo-wide); `theory` is the type for Lean proof work.

PR titles use the same format. We squash-merge, so the PR title becomes the
commit on `main` verbatim — a non-semantic PR title lands as a non-semantic
merge commit. Every commit follows the format above, including the pre-squash
commits inside a PR; both rules apply all the time.
