...
-- units and the 420-record harness and emits the paper's performance table
...
--   derived.
-- * The 420-record harness sweep pre-reads all inputs, then times one full
...
-- | Full 420-sweep passes; the median is reported.
sweepSamples :: Int
sweepSamples = 3
...
main = do
  ...
  unless (length inputs == 420) $
    die ("expected 420 manifest inputs, found " ++ show (length inputs))
  env <- gatherEnv
...
-- | One full pass over all 420 harness records, in memory: decode + check +
...
performanceTex env units sweepNs =
  unlines
    ...
    , texEsc (envLean env) ++ "."
    , "Haskell times are in-process medians of " ++ show samples
    , "~sections of " ++ show reps ++ "~batched runs each; end-to-end is"
  ...
  where
    ...
    texEsc = concatMap (\c -> if c `elem` "_%&#" then ['\\', c] else [c])
