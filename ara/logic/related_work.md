...
- **`rit`** (sibling project) — Lean kernel + sha256-pinned facts + AND/OR claim DAG; monotonic, no `gap`/`contested`/`defeated` distinction. Source-verified 2026-07-25 (C18): its world-facing fragment is five object kinds / six relations over log-extracted naturals discharged `by decide`, so paper claims enter through a silent defeasible narrowing LARA types; its non-kernel disagreement machinery (collisions, tiers, weakest-link roll-up) is an unproven shadow argumentation framework whose open aggregation problem grounded semantics answers.
