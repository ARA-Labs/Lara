...
- **Conditions**: Holds over a finite conclusion-labelled framework with a decidable attack relation, for the five semantics mechanized here (grounded, complete, preferred, stable, semi-stable). The agreement half additionally requires a duplicate-free carrier, which compilation supplies. The non-functionality theorem fixes preferred semantics and quantifies over every `AF → Arg → Status` candidate satisfying the two biconditionals; it does not claim that every semantics admits the two-cycle witness. Mutual exclusivity of the justified and defeated skeptical readings requires a non-empty extension set and conflict-freedom of every extension, which the interface does not require and which is therefore supplied per instance; at the preferred instance both halves are now discharged by theorem rather than by hypothesis (`preferredSem_enumerateConflictFree` and `preferredSem_enumerate_ne_nil`, the latter from `preferred_exists`, #196). The enumeration-based exclusivity theorems now avoid Nodup while retaining the spec-based compatibility API (#197). The mathematical interface remains open; the repository registry audit now detects unregistered named closed semantics declarations across modules (#198), including modules outside the library root imports. Parameterized families and local values remain outside that finite table.
...
- **Proof**: [`src/Lara/Driver/Internal.hs` (`buildCertOk`, `unitCertDeps`, `runCheckDeps`), `src/Lara/SupportTerm.hs` (`CertOutcome`, `certAccepted`), `src/Lara/Strict/Deps.hs`, `test/StrictSpec.hs` (`prop_mixedBackendDepsCollected`), `test/CliSpec.hs` (`prop_cliDepsAcceptS5`, `prop_cliDepsDoorsAgree`), issue #204, trace N247, N315, N316]
- **Dependencies**: []
...
- **Conditions**: Holds over the frozen compilation carrier at **any** extension semantics — originally proved for grounded only, and generalized in #216 with no additional hypothesis, because what the profile-preservation argument concludes is an *equality* of compiled carriers and every reader of a carrier inherits it (`obsGen_congr`, `backend_replacement_congruence_sem`). Requires admissible contexts — the link guard passes, both sides' declared material is well-formed relative to the *linked* environment, and the linked program passes the signature stage — whose own assurances satisfy `FixesContext`, and a forward-only acceptance-preservation hypothesis. Admissibility supplies an accepted source and preservation supplies an accepted target, so both detailed observations land in the observed arm; without it their rejection outcomes need not agree. The original family has leaf-name openness. The additive Context.Holes family (#217) represents named CQ-answer holes and proves typed substitution before the unchanged complete-program boundary, with context-fixing extended to filling certificates and attack coverage stated after substitution. Core-valued fillings support nested template holes; recursive filling environments and full abstraction are not claimed. Relational quantification over related backends is no longer an open boundary: #215 replaced the function `f` with a relation `R` and re-derived this theorem as the `graphOf f` instance (`backend_replacement_congruence_of_parametricity`, guarded by `congruence_correspondence`), so nothing here is now stated more generally than what is proved. Untested boundary: contexts carrying assurances moved by the relabel, two-way acceptance, how the equivalence relations compare among the standard non-grounded semantics or under identical-export restrictions, and a semantics-parametric congruence witness on a certificate-bearing fragment (#269). The unrestricted implication is refuted (#268) by ContextualSeparation.counterexample: all-context grounded equivalence and an accepted distinction under an adequate singleton-selector family. The disagreeing-carrier boundary (#270) is closed by `cycle_admissible` and `congruence_witness_sem`: admissibility and congruence now hold on the same cycle where grounded and stable observations differ. The fixture remains certificate-free. The composed and whole-program relational boundary (#277) is closed by `backend_replacement_parametricity_composed_sem` and `whole_program_parametricity_sem`; the latter preserves the already-checked contract without adding admissibility. `whole_program_parametricity_local_sem` supplies the separate closed-link occurrence-union instance.
...
- **Proof**: [`Lara.Context.crossAttsFrom_map`, `crossAtts_relabel`, `link_relabel_commutes`, `checkUnit_map`, `compileUnit_map`, `backend_replacement_congruence`, `registry_swap_congruence`, `link_attackComplete`, `link_checked`, `link_merge_status_eq`; witnesses `Lara.Examples.Linking.certAdmissible`, `cert_congruence_witness`, `cert_relabel_moves`, `cert_registry_swap_witness`; `ara/evidence/proofs/m4_contextual_adequacy.md`; `docs/theory-m4-contextual-adequacy.md`; PR #218, commits 78493f5, 776e50c, 0dc676c, 4c308c2, c61c91f. Generic instance (#216): `Lara.Context.obsGen_congr`, `backend_replacement_congruence_sem`, `registry_swap_congruence_sem`, `backend_replacement_congruence_composed_sem`, `whole_program_replacement_sem`; `ara/evidence/proofs/m4_generic_observation.md`; `docs/theory-m4-generic-observation.md`; PR #271, commits 3bc0e65, 40202bc, 02a64fd, 4dc223e; unified onto one observation type in commit eb6e955, which preserved every statement. Relational instance (#215): `Lara.Context.obsGen_parametricity`, `backend_replacement_parametricity{,_sem,_local,_local_sem}`, `backend_replacement_congruence_of_parametricity`, `congruence_correspondence`; `ara/evidence/proofs/m4_relational_parametricity.md`; `docs/theory-m4-relational-parametricity.md`; trace N337-N341; disagreeing-carrier instance (#270): `Lara.Examples.ContextSemantics.cycle_admissible`, `congruence_witness_sem`, `obsSem_cycle_stable_ne_grounded`; `ara/evidence/proofs/m4_cycle_admissibility.md`; #277: `ara/evidence/proofs/m4_relational_family_277.md`; `ara/evidence/proofs/lean_context_extensions_2026_09_09.md`; trace N277005, N277006]
- **Dependencies**: []
