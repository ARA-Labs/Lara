...
- **Conditions**: Holds over the frozen compilation carrier at **any** extension semantics — originally proved for grounded only, and generalized in #216 with no additional hypothesis, because what the profile-preservation argument concludes is an *equality* of compiled carriers and every reader of a carrier inherits it (`obsGen_congr`, `backend_replacement_congruence_sem`). Requires admissible contexts — the link guard passes, both sides' declared material is well-formed relative to the *linked* environment, and the linked program passes the signature stage — whose own assurances satisfy `FixesContext`, and a forward-only acceptance-preservation hypothesis. Admissibility supplies an accepted source and preservation supplies an accepted target, so both detailed observations land in the observed arm; without it their rejection outcomes need not agree. Openness is leaf-name openness only: term-level holes are unrepresentable because the compile boundary forces an empty obligation set on every declared argument. Untested boundary: contexts carrying assurances moved by the relabel, two-way acceptance, relational quantification over related backends (#215), how the *equivalence relations* at different semantics compare (#268 — the observation functions are separated, the relations are not), and a semantics-parametric congruence witness at a carrier where the semantics parameter is not inert (#270) or on a certificate-bearing fragment (#269).
...
- **Last revised**: 2026-09-07 (2026-09-07_001#4)
...
- **Conditions**: Established in one mechanized instance: the M4 contextual congruences, whose shared lemma `compileUnit_link_relabel` concludes `Invariants.compileUnit acc₂ = Invariants.compileUnit acc₁`, generalized from the grounded status to an arbitrary `Semantics.ExtensionSemantics` with hypotheses identical to the grounded statement's and no `AttackExtensional`. The contrasting case is internal to the same development: `Lara.Observation.observe_congr` transports between two distinct frameworks agreeing pointwise on their carrier and does take `AttackExtensional sem.spec`. The claim is about the *cost* of the generalization, not about its value — a family can be free to generalize and still be observing nothing new, which is why non-triviality needs separate witnesses at the generalized level. The sufficiency refinement was established separately, by landing both forms: the reader-parameterized family was first shipped with the generic type sitting *beside* the frozen one (PR #271), where `obs = obsGen (Invariants.status canon)` does not typecheck across the two types and the premise is prose; replacing the frozen inductive with an abbreviation of the generic one (commit eb6e955) makes the same equation close by `rfl`, and cost no `decide`-closed fixture because `abbrev` is definitionally transparent. Untested boundary: this is one instance in one development, and no claim is made that the equality/pointwise dichotomy exhausts the transport forms a shared lemma can take, nor that the equality case is always reachable by design rather than by luck; the replace-vs-sit-beside refinement likewise rests on this single pair of landed designs, in a language where `abbrev` is definitionally transparent.
