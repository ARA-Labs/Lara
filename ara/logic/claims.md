...
- **Sources**: ["`≡` is thus decidable, total, reflexive, symmetric, transitive, and linear in term size — a trivial addition to the TCB." ← docs/spec.md:362 «`≡` is thus decidable, total, reflexive, symmetric, transitive, and linear in term size» [input]", "8/8 properties pass ← evidence/status/test_status.md «prop ≡ reflexive … +++ OK, passed 100 tests» [result]"]
- **Status**: supported
...
- **Conditions**: Holds under a policy-declared `contrary` relation (not classical negation) and the
  ...
  question names.
- **Sources**: ["\"The three attack kinds are exactly the three kinds of positions in a term\" ← docs/spec.md:1083 «The three attack kinds are exactly the three kinds of positions in a term» [input]", "\"Attack checking is subterm-occurrence checking plus a contrary-relation lookup: decidable and local\" ← docs/spec.md:1108 «Attack checking is subterm-occurrence checking plus a contrary-relation lookup» [input]"]
- **Status**: supported
...
- **Sources**: ["\"a successful check discharges the deductive validity of this strict step, conditional on its premise conclusions and the declared theory; it does not establish that the premise support terms' conclusions are true\" ← docs/strict-backend-decision.md:43 «it does not establish that the premise support terms' conclusions are true» [input]", "Theorem 3 (source non-factivity), proved by inversion ← evidence/proofs/nonfactivity_and_defeat.md [result]"]
- **Status**: supported
...
- **Sources**: ["\"status cannot distinguish LP from another adapter with the same strict acceptance profile\" ← docs/strict-backend-decision.md:310 «status cannot distinguish LP from another adapter with the same strict acceptance profile» [input]", "Theorem 2 (backend replacement), proved by AF isomorphism + grounded-lfp invariance ← evidence/proofs/backend_replacement.md [result]", "backend_replacement mechanized (Model A, uniform injective relabel) ← lean/Lara/Erase.lean «theorem backend_replacement … statusC (checkedAF P₁) c = statusC (checkedAF P₂) c» [result]", "0 sorryAx across 509 declarations, axioms ⊆ trio ← lean AxCheck run 2026-07-30 «sorryAx count: 0 / none — all within the trio» [result]"]
- **Status**: supported
...
- **Sources**: ["\"adding evidence or attacks can retract a claim's justified status even though the original support term remains well typed\" ← docs/spec.md:1277 «adding evidence or attacks can retract a claim's `justified` status even though the» [input]", "Proposition 8 (monotonic consequence cannot represent defeat-driven retraction), proved by a subset counterexample ← evidence/proofs/nonfactivity_and_defeat.md [result]"]
- **Status**: supported
...
- **Sources**: ["\"the ascending chain stabilizes after at most `|Args|` strict-growth steps, so grounded evaluation is deterministic and terminating; attack cycles produce `undec` labels rather than nontermination\" ← docs/spec.md:1272-1273 «grounded evaluation is deterministic and terminating; attack cycles produce `undec` labels rather than nontermination» [input]"]
- **Status**: supported
...
- **Evidence basis**: spec §8 gives the fixed-point argument; `lean/Lara/Grounded.lean` proves
  ...
  calculus). Subsequent work supplies checked-unit attack completeness and result 7.
  `lean/AxCheck.lean` audits these theorems without `sorryAx`.
- **Dependencies**: C06
- **Tags**: grounded-semantics, determinism, termination, four-state-status
- **Last revised**: 2026-07-25 (`Faithful` oracle discharged by `edgeB_faithful`)
...
- **Sources**: ["\"The former accountability theorem … is thereby an inversion lemma on term structure: the reported leaf dependency set is exactly `leaves(w)`.\" ← docs/spec.md:941 «the reported leaf dependency set is exactly `leaves(w)`» [input]"]
- **Status**: supported
...
- **Evidence basis**: spec §6 defines `leaves(w)`/`certDeps(w)`;
  `lean/Lara/Support.lean` proves `leaves_declared` for the source-leaf half, and (as of 2026-07-31)
...
- **Last revised**: 2026-07-31 (certificate-half mechanization, fixed-core/data-only-digest registry; status testing → supported)
...
- **Sources**: ["\"strict closure introduces no new conflict and direct = indirect consistency hold by construction — two contrary claims are never jointly justified\" ← docs/spec.md:1339 «introduces no new conflict and direct = indirect consistency hold by construction — two contrary» [input]"]
- **Status**: supported
...
- **Sources**: ["\"it reports high acceptance on correct instances and zero false positives on deliberately-incorrect (adversarial) instances\" ← docs/worked-examples-plan.md:204 «zero false positives on deliberately-incorrect (adversarial)» [input]", "\"An untrusted LLM front-end (added later, in Python) proposes certificates; the checker is the sole arbiter of structural validity.\" ← docs/substrate-decision.md:17-19 «Python emits claim-support programs, opaque strict-certificate payloads, and leaf atoms; the Haskell checker decides validity.» [input]"]
- **Status**: hypothesis
...
- **Conditions**: Holds for the Haskell↔Lean cross-check once both share one serialized first-order
  ...
  Bounded, as of trace N331_gate_controls, by what the anchor set reaches: the localizing power is the framing's, but a
  ...
  Since the reader totalization that remedy is no longer symmetric in kind: the Lean half of the boundary is proved,
...
- **Proof**: [trace N96 (T3 smoke: location-accuracy 246/246 over the single-defect manifest), docs/m5-freeze-checklist.md (T6 scoped confirmatory-by-construction on this ground)]
- **Dependencies**: []
...
- **Conditions**: Seeded mutation corpus; ablation cells `noCQConfig`, `noTypedConfig`, and `noConflictScanConfig` of `Lara.Check.CheckConfig`. Each disables its documented obligations; `noTypedConfig` disables both typed attacks and conflict-scan completeness, so it maps to R10/R11/MissingConflict together. Surgicality and monotonicity are checked by `AblationSpec`. Untested boundary: human-authored natural defects, deferred; no claim about naturally occurring defect rates.
- **Sources**: ["18/637 ← ara/evidence/results/backend_mutation_v6.md «no-cq: count=655; missed-rejects=18; unchanged=637; missed classes: reject-IncompleteArgument=18.» [result]", "37/618 ← ara/evidence/results/backend_mutation_v6.md «no-typed: count=655; missed-rejects=37; unchanged=618; missed classes: reject-MissingConflict=5, reject-R10=11, reject-R11=21.» [result]", "5/650 ← ara/evidence/results/backend_mutation_v6.md «no-conflict-scan: count=655; missed-rejects=5; unchanged=650; missed classes: reject-MissingConflict=5.» [result]"]
- **Status**: supported
- **Provenance**: ai-suggested
...
- **Proof**: [`measurements/frozen/ablation.{json,tsv}` (clean bc888a5 v6 run), `ara/evidence/results/backend_mutation_v6.md`, `test/AblationSpec.hs`, trace N266_v6; historical T6 execution remains recorded in N97]
- **Dependencies**: []
- **Tags**: evaluation, ablation, rule-necessity, axis-a, confirmatory-by-construction
- **Last revised**: 2026-09-09 (2026-09-09_001#4)
...
- **Sources**: [
  `result 13` ← `docs/mechanization-plan.md` «| 13 | Well-sortedness is decidable and preserved by rule instantiation (`lara-core@0.2`, issue [redacted]) |» [result];
...
- **Proof**: [`ara/evidence/results/parsetag_map_lookup.md`, `src/Lara/Wire.hs`, `test/WireSpec.hs`, `scripts/differential.sh`, trace N187, trace N188, commit `a7b498a`]
- **Dependencies**: []
...
- **Proof**: [`ara/evidence/results/bytestring_wire_parser.md`, `src/Lara/Wire.hs`, `test/WireSpec.hs` (`referenceParseSExpr` differential over 581 committed inputs plus eleven exact position pins), `scripts/differential.sh`, `docs/m5-freeze-checklist.md`, trace N191, trace N192, trace N193, commit `a2904d7`]
- **Dependencies**: [C32]
...
- **Conditions**: Holds over a finite conclusion-labelled framework with a decidable attack relation, for the five semantics mechanized here (grounded, complete, preferred, stable, semi-stable). The agreement half additionally requires a duplicate-free carrier, which compilation supplies. The non-functionality theorem fixes preferred semantics and quantifies over every `AF → Arg → Status` candidate satisfying the two biconditionals; it does not claim that every semantics admits the two-cycle witness. Mutual exclusivity of the justified and defeated skeptical readings requires a non-empty extension set and conflict-freedom of every extension, which the interface does not require and which is therefore supplied per instance; at the preferred instance both halves are now discharged by theorem rather than by hypothesis (`preferredSem_enumerateConflictFree` and `preferredSem_enumerate_ne_nil`, the latter from `preferred_exists`). The enumeration-based exclusivity theorems now avoid Nodup while retaining the spec-based compatibility API. The mathematical interface remains open; the repository registry audit now detects unregistered named closed semantics declarations across modules, including modules outside the library root imports. Parameterized families and local values remain outside that finite table.
...
- **Proof**: [`lean/Lara/Semantics.lean` (`observe_grounded`, `groundedSem_enumerate`, `observe_gap`, `justified_defeated_exclusive`, the five `*_specConflictFree`, `preferred_exists` / `preferredSem_enumerate_ne_nil`), `lean/Lara/Examples/Semantics.lean` (`credulous_not_functional`, `profile_preferredSem_twoCycle_symm`, `observe_not_determined_by_profile`, `five_semantics_pairwise_distinct`), `lean/AxCheck.lean`, `docs/theory-m2a-observation.md`, trace N230, trace N232, trace N236; `ara/evidence/proofs/lean_context_extensions_2026_09_09.md`; trace N277003, N277004]
- **Dependencies**: []
- **Tags**: mechanization, extension-semantics, claim-status, credulous-skeptical, non-collapse, M2a
- **Last revised**: 2026-09-09 (2026-09-09_001#3)
...
- **Sources**: [`sink boundary` ← `docs/theory-m3-source-updates.md:245-246` «A fresh instance is a sink in the old framework: freshness prevents old raw attacks from naming it, while old-to-old edges and labels are preserved.» [result]; `axiom set` ← `lean/AxCheck.lean` via `scripts/check-axioms.sh` «Axiom audit passed.» [result]]
- **Status**: supported
- **Provenance**: ai-suggested
...
- **Sources**: [`clean-base definition` ← `docs/theory-m3-source-updates.md:143-144` «`Lara.Update.CleanBase run` means exactly `run.admission.prune.removedSeed = []` for the source accepted run.» [input]; `unconditional counterexample` ← `docs/theory-m3-source-updates.md:170-171` «This witness refutes an unconditional additive/core equality theorem; it does not make `CleanBase` necessary for every individual run.» [result]]
- **Status**: supported
- **Provenance**: ai-suggested
...
- **Sources**: [`preservation premise` ← `docs/theory-m3-source-updates.md:89-92` «`applyUpdate_addInstance_ok` assumes an accepted source, name-and-term freshness, well-sortedness of the new term, a complete support derivation for it, and `Compile.AttackComplete` for the old retained arguments extended by that term.» [input]; `non-unconditional boundary` ← `docs/theory-m3-source-updates.md:95` «None says that its constructor always succeeds.» [result]]
- **Status**: supported
- **Provenance**: ai-suggested
...
- **Proof**: [`src/Lara/Driver/Internal.hs` (`buildCertOk`, `unitCertDeps`, `runCheckDeps`), `src/Lara/SupportTerm.hs` (`CertOutcome`, `certAccepted`), `src/Lara/Strict/Deps.hs`, `test/StrictSpec.hs` (`prop_mixedBackendDepsCollected`), `test/CliSpec.hs` (`prop_cliDepsAcceptS5`, `prop_cliDepsDoorsAgree`), trace N247, N315, N316]
- **Dependencies**: []
- **Tags**: mechanization, conformance, production-parity, dependency-accounting, B0
- **Last revised**: 2026-09-06 (2026-09-06_001#3)
...
- **Proof**: [`Lara.Examples.PW.t7_witness`, `t7_src_justified`, `t7_tgt_defeated`, `t7_transport_wellFormed`, `t7_support_transported`, `t7_dia_defeated`, `t7_box_defeated`, `t7_dia_defeated_src`, `t7_box_defeated_src`; `evidence/proofs/pw0_outer_model.md`; commits 817824f and 86c5163; axiom audit 1619 declarations PASS]
- **Dependencies**: []
...
- **Conditions**: Holds over the frozen compilation carrier at **any** extension semantics — originally proved for grounded only, and later generalized with no additional hypothesis, because what the profile-preservation argument concludes is an *equality* of compiled carriers and every reader of a carrier inherits it (`obsGen_congr`, `backend_replacement_congruence_sem`). Requires admissible contexts — the link guard passes, both sides' declared material is well-formed relative to the *linked* environment, and the linked program passes the signature stage — whose own assurances satisfy `FixesContext`, and a forward-only acceptance-preservation hypothesis. Admissibility supplies an accepted source and preservation supplies an accepted target, so both detailed observations land in the observed arm; without it their rejection outcomes need not agree. The original family has leaf-name openness. The additive Context.Holes family represents named CQ-answer holes and proves typed substitution before the unchanged complete-program boundary, with context-fixing extended to filling certificates and attack coverage stated after substitution. Core-valued fillings support nested template holes; recursive filling environments and full abstraction are not claimed. Relational quantification over related backends is no longer an open boundary: the function `f` was replaced with a relation `R` and the theorem re-derived as the `graphOf f` instance (`backend_replacement_congruence_of_parametricity`, guarded by `congruence_correspondence`), so nothing here is now stated more generally than what is proved. Untested boundary: contexts carrying assurances moved by the relabel, two-way acceptance, how the equivalence relations compare among the standard non-grounded semantics or under identical-export restrictions, and a semantics-parametric congruence witness on a certificate-bearing fragment. The unrestricted implication is refuted by ContextualSeparation.counterexample: all-context grounded equivalence and an accepted distinction under an adequate singleton-selector family. The disagreeing-carrier boundary is closed by `cycle_admissible` and `congruence_witness_sem`: admissibility and congruence now hold on the same cycle where grounded and stable observations differ. The fixture remains certificate-free. The composed and whole-program relational boundary is closed by `backend_replacement_parametricity_composed_sem` and `whole_program_parametricity_sem`; the latter preserves the already-checked contract without adding admissibility. `whole_program_parametricity_local_sem` supplies the separate closed-link occurrence-union instance.
...
- **Proof**: [`Lara.Context.crossAttsFrom_map`, `crossAtts_relabel`, `link_relabel_commutes`, `checkUnit_map`, `compileUnit_map`, `backend_replacement_congruence`, `registry_swap_congruence`, `link_attackComplete`, `link_checked`, `link_merge_status_eq`; witnesses `Lara.Examples.Linking.certAdmissible`, `cert_congruence_witness`, `cert_relabel_moves`, `cert_registry_swap_witness`; `ara/evidence/proofs/m4_contextual_adequacy.md`; `docs/theory-m4-contextual-adequacy.md`; commits 78493f5, 776e50c, 0dc676c, 4c308c2, c61c91f. Generic instance: `Lara.Context.obsGen_congr`, `backend_replacement_congruence_sem`, `registry_swap_congruence_sem`, `backend_replacement_congruence_composed_sem`, `whole_program_replacement_sem`; `ara/evidence/proofs/m4_generic_observation.md`; `docs/theory-m4-generic-observation.md`; commits 3bc0e65, 40202bc, 02a64fd, 4dc223e; unified onto one observation type in commit eb6e955, which preserved every statement. Relational instance: `Lara.Context.obsGen_parametricity`, `backend_replacement_parametricity{,_sem,_local,_local_sem}`, `backend_replacement_congruence_of_parametricity`, `congruence_correspondence`; `ara/evidence/proofs/m4_relational_parametricity.md`; `docs/theory-m4-relational-parametricity.md`; trace N337-N341; disagreeing-carrier instance: `Lara.Examples.ContextSemantics.cycle_admissible`, `congruence_witness_sem`, `obsSem_cycle_stable_ne_grounded`; `ara/evidence/proofs/m4_cycle_admissibility.md`; `ara/evidence/proofs/m4_relational_family_277.md`; `ara/evidence/proofs/lean_context_extensions_2026_09_09.md`; trace N277005, N277006]
- **Dependencies**: []
- **Tags**: M4, context-calculus, backend-replacement, congruence, representation-independence, saturation, mechanized, extension-semantics
- **Last revised**: 2026-09-09 (2026-09-09_001#3)
...
- **Conditions**: Shown for the bridge-global, functional partial translation and total leaf renaming of T6, over a shared canonicalizer, with attack forth/back stated on the compiled edge decider `Compile.edgeB` (index level) rather than derived from a correspondence of declared attacks. Injectivity-where-defined of the symbol translation is required for the derived support correspondence; bridges that supply the correspondence by other means use `status_transport_of_corr` without it. Executable cells cover `justified`, `defeated`, `gap`, and `contested` under one renaming translation between contexts with disjoint signatures, and one `[b]` collapse. Untested boundary: status under non-injective (merging) translations; the general injectivity-free derivation of the attack clauses from declared attacks (`AttackBridge.toStatusBridge` narrows it — deriving `forth`/`back` from declared-attack transport, but under an extra `Function.Injective lm` hypothesis the frozen contract does not require); T6 limitations 1, 2, and 5 inherited. A follow-up discharges the decider ask: `statusBridgeB` with `statusBridgeB_sound`/`statusBridgeB_complete` (`Lara/PW/StatusCheck.lean`), each conformance cell one `decide`.
- **Sources**: [`inj consumed by no proof` ← `ara/evidence/proofs/pw_t8_status_preservation.md:18` «field is consumed by no proof, so a surjective bounded morphism already» [result]; `143-job build` ← `ara/evidence/proofs/pw_t8_status_preservation.md:61` «Build completed successfully (143 jobs).» [result]; `60 gated declarations` ← `ara/evidence/proofs/pw_t8_status_preservation.md:65` «AxCheck coverage passed (60 declarations).» [result]; `2058 audited, 60 PW-T8` ← `ara/evidence/proofs/pw_t8_status_preservation.md:71` «The audit reports 2058 declarations across the library, of which 60 are PW-T8» [result]; `Classical.choice via labelC_spec` ← `ara/evidence/proofs/pw_t8_status_preservation.md:75` «`Classical.choice` reaches the T8 rows through `Grounded.labelC_spec`; the» [result]; `hSB and hinj both necessary` ← `ara/evidence/proofs/pw_t8_status_preservation.md:81` «`hinj` from `status_transport`'s signature leaves an unfillable goal» [result]; `contested cell admissible` ← `ara/evidence/proofs/pw_t8_status_preservation.md:53` «strict-reachable rule conclusions. `t8_box_defeated_r` instantiates the `[b]`» [result]]
- **Status**: supported
- **Provenance**: ai-suggested
...
- **Proof**: [`Lara.PW.AttackBisim`, `labelC_of_bisim`, `statusC_congr`, `statusC_of_bisim`, `AFIso.toBisim`, `statusC_of_iso`, `supportCorr_of_image`; `Lara.PW.StatusBridge`, `StatusBridge.bisim`, `corr_conclusion`, `claimSupport_corr`, `equiv_tr_reflect`, `status_transport`, `status_transport_of_corr`, `srcStatus_transport`, `sat_status_iff_box`, `sat_status_iff_dia`, `sat_status_iff_box_src`, `StatusBridge.comp`, `corr_comp_iff`; witnesses `Lara.Examples.PW.Status.t7_forward_hom_insufficient`, `t7_not_statusBridge`, `t7_not_statusBridge_of_flip`, `t8_justified_preserved`, `t8_defeated_preserved`, `t8_gap_preserved`, `t8_contested_preserved`, `t8_box_defeated_r_holds`; `ara/evidence/proofs/pw_t8_status_preservation.md`; `docs/theory-pw-t8-status-preservation.md`; commits 424e4e7..5637540; axiom audit 2058 declarations PASS; follow-ups `Lara.PW.AttackBridge.toStatusBridge`, `statusBridgeB_sound`, `statusBridgeB_complete`, `Lara.PW.zipOpt_bind`, witnesses `Lara.Examples.PW.Attack.s2_r2_attackBridge`, `gap_attack_undefined`, `collapsed_containsB_breaks`, `ques_position_law`, `Lara.Examples.PW.StatusCheck.t7_not_statusBridge_via_decider`; `ara/evidence/proofs/pw_t8_t9_followups.md`; axiom audit 2304 declarations PASS]
...
## C52: A family of results generalizes along a parameter for free exactly when its shared lemma concludes an equality, not a pointwise agreement
- **Statement**: When every result in a family funnels through one lemma, the cost of generalizing the family along a new parameter is decided by that lemma's *conclusion form*. If it concludes an equality of the object the results read, then any reader of that object inherits the family with no additional hypothesis, because both sides of the generalized statement apply the reader to the same argument. If it concludes only that two distinct objects agree pointwise, the generalization needs an extensionality hypothesis to license transporting the reading across them. The two are not interchangeable: importing the extensionality hypothesis into an equality-transported generalization yields a statement strictly weaker than the specific result it claims to generalize, so it fails to be a generalization at all. The design corollary is to parameterize the *reader* rather than the concept the parameter belongs to — that proves the family once and turns the observation itself into a theorem rather than a comment. Parameterizing the reader is necessary but not sufficient for that last part: the generic and specific result types must be the *same* type, or the collapse relating them is not even well-formed and the premise stays a comment after all.
- **Conditions**: Established in one mechanized instance: the M4 contextual congruences, whose shared lemma `compileUnit_link_relabel` concludes `Invariants.compileUnit acc₂ = Invariants.compileUnit acc₁`, generalized from the grounded status to an arbitrary `Semantics.ExtensionSemantics` with hypotheses identical to the grounded statement's and no `AttackExtensional`. The contrasting case is internal to the same development: `Lara.Observation.observe_congr` transports between two distinct frameworks agreeing pointwise on their carrier and does take `AttackExtensional sem.spec`. The claim is about the *cost* of the generalization, not about its value — a family can be free to generalize and still be observing nothing new, which is why non-triviality needs separate witnesses at the generalized level. A second instance landed later, along a different axis: the same shared lemma's relational counterpart `Lara.Context.compileUnit_rel` again concludes an equality of compiled carriers, and the M4 congruence generalizes from a *function* on certificates to a *relation* with no grounded-labelling lemma, no `AttackExtensional` hypothesis and no new fixpoint machinery — so the equality/pointwise dichotomy predicts the cost of generalizing along the relating structure as well as along the reader. The sufficiency refinement was established separately, by landing both forms: the reader-parameterized family was first shipped with the generic type sitting *beside* the frozen one, where `obs = obsGen (Invariants.status canon)` does not typecheck across the two types and the premise is prose; replacing the frozen inductive with an abbreviation of the generic one (commit eb6e955) makes the same equation close by `rfl`, and cost no `decide`-closed fixture because `abbrev` is definitionally transparent. Untested boundary: two instances in one development, both of which inherit the equality from the same original lemma, so they are not independent evidence that the dichotomy holds generally; and no claim is made that the equality/pointwise dichotomy exhausts the transport forms a shared lemma can take, nor that the equality case is always reachable by design rather than by luck; the replace-vs-sit-beside refinement likewise rests on this single pair of landed designs, in a language where `abbrev` is definitionally transparent.
- **Sources**: [`154-job build` ← `ara/evidence/proofs/m4_generic_observation.md` «Build completed successfully (154 jobs).» [result]; `coverage outcome` ← `ara/evidence/proofs/m4_generic_observation.md` «AxCheck coverage passed (37 declarations).» [result]; `audit outcome` ← `ara/evidence/proofs/m4_generic_observation.md` «Axiom audit passed.» [result]; `hypothesis parity` ← `ara/evidence/proofs/m4_generic_observation.md` «Hypotheses are **exactly** `backend_replacement_congruence`'s» [result]; `contrasting case takes the hypothesis` ← `ara/evidence/proofs/m4_generic_observation.md` «does take `(hext : AttackExtensional sem.spec)`» [result]; `the collapse is unstateable across two types` ← `ara/evidence/proofs/m4_generic_observation.md` «This does not typecheck under the two-type design: `obs : Observation` and» [result]; `unified-type build` ← `ara/evidence/proofs/m4_generic_observation.md` «Build completed successfully (100 jobs).» [result]; `fixtures survived the replacement` ← `ara/evidence/proofs/m4_generic_observation.md` «`congruence_witness` (`:676`) and `registry_swap_witness` (`:670`) compile» [result]]
- **Status**: testing
- **Provenance**: ai-suggested
- **Falsification**: Exhibit a family funnelling through an equality-concluding shared lemma that nonetheless requires a new hypothesis to generalize along a reader of the equated object; or exhibit a pointwise-agreement-concluding shared lemma whose family generalizes with no extensionality hypothesis; or show that `obsGen_congr` in fact depends on an extensionality property smuggled in through `compileUnit_link_relabel`'s own hypotheses, which would collapse the distinction this claim draws.
- **Proof**: [`Lara.Context.obsGen_congr` and its four instantiations, `lean/Lara/Context/Equivalence.lean:784` (the instantiations in `lean/Lara/Context/Observation.lean`); `Lara.Context.obs_eq_obsGen`, `lean/Lara/Context/Equivalence.lean:665` (the collapse, by `rfl`); `Lara.Context.compileUnit_link_relabel`, `lean/Lara/Context/Equivalence.lean:712`; `Lara.Observation.observe_congr`, `lean/Lara/Observation.lean:406`; `Lara.Invariants.observeSem_grounded` (the companion unconditional `Nodup` discharge); `ara/evidence/proofs/m4_generic_observation.md`; `docs/theory-m4-generic-observation.md` §2-§3; commit eb6e955; trace N327, N328, N333, N334. Second instance: `Lara.Context.compileUnit_rel`, `obsGen_parametricity`, `backend_replacement_parametricity_sem`, `lean/Lara/Context/Parametricity.lean`; `ara/evidence/proofs/m4_relational_parametricity.md`; `docs/theory-m4-relational-parametricity.md` §5; trace N338]
- **Dependencies**: [C49, C35]
- **Tags**: mechanization, generalization, congruence, extension-semantics, proof-engineering, M4, M2a
- **Last revised**: 2026-09-08 (2026-09-08_001#1)

## C53: Finite related material has a functional realization; acceptance scope remains an independent obligation
- **Statement**: A partial-injective certificate relation on finite structured material can be realized by a total injective relabeling, simultaneously fixing finite related context material. Consequently functional congruence's structural premises can be discharged for the same fragments and context. Relational acceptance preservation is guarded by the relation; global functional acceptance preservation remains a separate obligation on the realizing map. This finite realization does not extend all pairs of an arbitrary infinite relation.
- **Conditions**: Mechanized for M4 fragments and contexts under `RelInj`, `RelFrag`, and `RelFixesContext`. `exists_total_injective_extension_on` restricts agreement to a finite list of source assurances; `relFrag_exists_injective_fixesContext` uses the combined fragment/context occurrences. The unrestricted extension assertion is refuted on `Assurance` by the inverse graph of the certificate-version shift. The occurrence-local congruence instantiates guarded acceptance but is not itself a witness that every globally preserving realization fails. The observational necessity of keeping an equality-reflection hypothesis is separately witnessed by `Examples.CertificateCollapse.relInj_observationally_necessary`: a functional but non-injective relation satisfies all other premises and changes an accepted grounded observation. This does not separate the relational theorem's strength from its functional counterpart. No claim is made about calculi with weaker relational side conditions or infinite structured material.
- **Sources**: [`finite realization` ← `lean/Lara/Context/FiniteExtension.lean` «theorem relFrag_exists_injective_fixesContext» [result]; `unrestricted counterexample` ← `lean/Lara/Context/FiniteExtension.lean` «theorem not_every_relInj_has_total_extension» [result]]
- **Status**: hypothesis
- **Provenance**: ai-suggested
- **Falsification**: Exhibit finite RelInj-related fragment and context material for which no injective relabeling both realizes the target fragment and fixes the context; or show that structural realization alone entails global acceptance preservation without any acceptance assumptions.
- **Proof**: [`lean/Lara/Context/FiniteExtension.lean`: `exists_total_injective_extension_on`, `relFrag_exists_injective`, `relFrag_exists_injective_fixesContext`, `not_every_relInj_has_total_extension`; `lean/Lara/Context/Parametricity.lean`: `backend_replacement_parametricity_local`; `ara/evidence/proofs/m4_finite_extension_279.md`; `docs/theory-m4-relational-parametricity.md` §4; trace N279-extension; `lean/Lara/Examples/CertificateCollapse.lean`: `relInj_observationally_necessary`; `ara/evidence/proofs/m4_certificate_collapse.md`; trace N275_observational_collapse]
- **Dependencies**: [C49]
- **Tags**: M4, parametricity, representation-independence, relational-reasoning, backend-replacement, mechanized
- **Last revised**: 2026-09-09 (2026-09-09_001#1)

## C54: A structural relation must reflect equality exactly where the calculus decides equality, and that requirement is a fact about the theorem rather than about the proof
- **Statement**: Lifting a relation on leaf payloads structurally through a term language is unconstrained until some downstream stage compares terms for equality. Each such comparison — a duplicate-eliminating merge, an equality-deciding coverage test, a distinctness premise on a merged list — forces the relation to reflect and preserve equality, because a relation that collapses two distinct terms changes the merged list's length, adds edges the original did not have, or destroys the distinctness the premise requires. The requirement is therefore locatable in advance by enumerating the calculus's equality decisions, and it is a property of the statement rather than of the proof strategy: the demonstration is to take the lemma that consumes the condition, delete the condition, and negate the conclusion — if that is provable, no proof of the lemma can drop the condition. Stating the necessity this way, rather than as two unconnected computations showing a decider is non-constant, is what makes it a witness at all.
- **Conditions**: Established in one mechanized instance: `RelInj` in M4's relational congruence, with exactly three consumers — `dedupList` (`Context/Fragment.lean:68`), `coveredB` (`Compile.lean:441`) and `checkUnit_complete`'s `Nodup` premise — and `relInj_necessary` as the negated-`coveredB_rel` witness. `Examples.CertificateCollapse.relInj_observationally_necessary` additionally witnesses that removing `RelInj` from the observation theorem permits an accepted export to change from justified to defeated; the graph of the collapse remains single-valued, so this does not establish necessity of each half separately. The enumerate-the-equality-decisions method was applied prospectively here (the three consumers were named in the plan before any proof was written) and all three turned out to be consumers, but the enumeration was over a calculus whose equality decisions are few and syntactically obvious. Untested boundary: whether the method scales to a calculus where equality is decided behind an abstraction barrier, and whether a *weaker* reflection condition suffices at any individual consumer — the three were discharged by one shared condition, not by three tailored ones.
- **Sources**: [`three consumers` ← `docs/theory-m4-relational-parametricity.md` «Three consumers force it, and they are not proof-engineering conveniences» [result]; `witness shape` ← `ara/evidence/proofs/m4_relational_parametricity.md` «`Context.relInj_necessary` is `coveredB_rel` with `hR` deleted and the conclusion negated» [result]; `audit outcome` ← `ara/evidence/proofs/m4_relational_parametricity.md` «Axiom audit passed.» [result]]
- **Status**: hypothesis
- **Provenance**: ai-suggested
- **Falsification**: Exhibit a stage in this calculus that decides term equality and yet transports along a non-reflecting relation — which would show the enumeration over-approximates; or prove `coveredB_rel` without `RelInj`, which would contradict `relInj_necessary` and mean one of the two is wrong; or exhibit a relational lifting whose necessary side condition is *not* locatable from the calculus's equality decisions, e.g. forced instead by a numeric or ordering comparison, which would show the enumeration under-approximates.
- **Proof**: [`Lara.Context.RelInj`, `relInj_necessary`, `dedupList_rel`, `coveredB_rel`, `nodup_rel`, `relTerm_inj`, `lean/Lara/Context/Parametricity.lean`; `lean/Lara/Erase.lean` header (the pre-existing record that a non-injective erase adds subargument-closure edges); `ara/evidence/proofs/m4_relational_parametricity.md`; `docs/theory-m4-relational-parametricity.md` §3; trace N337, N339, N275_observational_collapse; `ara/evidence/proofs/m4_certificate_collapse.md`]
- **Dependencies**: [C53]
- **Tags**: M4, relational-reasoning, side-conditions, necessity-witness, mechanized, proof-engineering

## C55: A wrapper over an unchanged local judgment can separate exactly the reporting conditions decided before that judgment runs; the rest are the local layer's to split
- **Statement**: When an outer layer reports on a subject by consulting an unchanged local observation, the conditions its report conflates fall into two kinds, and only one is the wrapper's to fix. Conditions decidable from the *static environment* — the vocabulary and typing a query must satisfy to be asked at all — can be lifted out of the report by refining the query type, because the check runs before any local judgment is consulted and its verdict is therefore independent of the subject being observed. Conditions the local observation function itself collapses cannot: if that function factors through a projection that discards the distinction, no wrapper-side refinement recovers it, and separating them requires changing the local layer — which is exactly what a wrapper whose value rests on redefining nothing local is forbidden to do. The practical consequence is that a "conflated report" limitation should be split by this test before any work is planned, because the two halves have different costs and one of them may be unavailable at any price the layer can pay.
- **Conditions**: Established in one mechanized instance: PW0's `gap`, whose limitation records four conflated conditions. The two static ones (out of vocabulary, ill-sorted) lift out by refining `Query_κ` to well-sorted claims over `Σ_κ`, with `notPosable_world_independent` witnessing that the verdict consults no world. The two dynamic ones (not posed, posed but unsupported) collapse because `Grounded.statusC` gaps exactly on empty complete support and every argument an accepted unit retains is complete, so an addressed claim cannot gap. Holds where the wrapper's discipline forbids redefining local judgments (PW0 gate 1) and where the static check is genuinely a function of the environment alone. Untested boundary: one instance, in a development whose local observation is a total function of a two-field projection with one field definitionally empty; nothing here shows the dichotomy is exhaustive, nor that a *partially* discarding projection behaves the same way, nor that the static/dynamic line always falls where the type system happens to draw it. The refinement's harmlessness itself needed a side condition (`SortedWorld`) that is not automatic, which is evidence that "decidable from the static environment" is a claim about the *environment as constrained*, not about the syntax.
- **Sources**: [`four conflated conditions` ← `docs/theory-pw0-outer-model.md` «**2. `gap` conflates four conditions** — *out of vocabulary*, *ill-sorted*,» [input]; `two lift out, two collapse` ← `ara/evidence/proofs/pw_sorted_queries_307.md` «Net: four conflated conditions become **two reported ones plus a proved» [result]; `the collapsing theorem` ← `ara/evidence/proofs/pw_sorted_queries_307.md` «theorem statusC_gap_iff (F : AF) (c : Claim) : statusC F c = Status.gap ↔ c.support = []» [result]; `verdict consults no world` ← `ara/evidence/proofs/pw_sorted_queries_307.md` «Same verdict for *any* candidate list, acceptance predicate and status» [result]; `audit outcome` ← `ara/evidence/proofs/pw_sorted_queries_307.md` «Axiom audit passed.» [result]; `coverage outcome` ← `ara/evidence/proofs/pw_sorted_queries_307.md` «AxCheck coverage passed (2934 declarations).» [result]]
- **Status**: hypothesis
- **Provenance**: ai-suggested
- **Falsification**: Exhibit a wrapper-side refinement that separates two conditions the unchanged local observation function maps to the same value, without changing any local definition — which would show the collapse is a fact about this design rather than about the layering. Or exhibit a statically-decidable conflated condition that a query-type refinement provably cannot lift out, which would show the first half over-reaches. Or show that `not_gap_of_addresses` in fact depends on a property of `Consistency.completeClaimFor` that could be changed *within* the wrapper's discipline, which would collapse the distinction the claim draws between the two kinds.
- **Proof**: [`Lara.PW.Sorted.cmpStatus_gap_iff_not_addresses`, `not_gap_of_addresses`, `notPosable_world_independent`, `report_ne_observed_iff_gap`, `sortedWorld_of_nodes`, `lean/Lara/PW/Sorted.lean`; `Lara.Grounded.statusC_gap_iff`, `lean/Lara/Grounded.lean`; `Compile.CheckedNode.valid` and `Unit.CheckedUnit.nodes_terms` (every retained argument is complete), `lean/Lara/Compile.lean`, `lean/Lara/Unit.lean`; the three-report witness `Lara.Examples.PWSorted.reports_distinct`, `split_not_change`, `lean/Lara/Examples/PWSorted.lean`; `ara/evidence/proofs/pw_sorted_queries_307.md`; `docs/theory-pw-sorted-queries.md` §4; `docs/theory-pw0-outer-model.md` limitation 2; trace N307_sorted, N307_gap_collapse, N307_sorted_world]
- **Dependencies**: []
