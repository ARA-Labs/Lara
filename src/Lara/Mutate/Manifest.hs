...
import Lara.Diagnostics (seededSitesText)
import Lara.Mutate
  ( Expected (..)
  , Mutant (..)
  , codecDiagnostics
...
-- 'seededSitesText' as column 8 (@-@ when there is no seeded site; a
  ...
  unlines
    ( "# file\tbase\tfamily\toperator\texpected\ths-diagnostic\tlean-diagnostic\texpected-location"
        : [ mutantPath m
              ...
              ++ mutantBase m
              ++ "\t"
              ++ familyText (opFamily (mutantOp m))
              ++ "\t"
              ++ opName (mutantOp m)
              ++ "\t"
