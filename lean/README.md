...
| **12** (presentation-codec round trip) | `Lara/Presentation.lean` | ✅ **mechanized at `lara-syntax@0.3` parity** — structured encode/decode round trips every `Program` (5) and `Policy` (9) field and every `Decl` arm (7), including `@0.3`'s `measurand`/`comparison-scheme`/`comparison` constructs, the surface attack step, and `Rule`'s `rulePremiseLabels`. The module's own `## Scope` header enumerates coverage and names what is deliberately out of scope; prefer it over this row. This anchors AST shape, not the concrete Haskell `.lara` parser; the latter is covered separately by QuickCheck conformance. |
