...
| **12** (presentation-codec round trip) | `Lara/Presentation.lean` | ◐ **structured subset mechanized; current Haskell surface covered** — Lean structured encode/decode round-trips its modeled `Program`/`Policy` fields and `@0.3` constructs, but does not yet include `policySigma` or optional measurand polarity. Haskell QuickCheck covers the current concrete `.lara` parser/printer, including those fields. This anchors modeled AST shape, not the concrete parser. |
