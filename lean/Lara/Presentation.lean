...
`src/Lara/AST.hs` at `lara-syntax@0.9` — every field of both top-levels,
...
`src/Lara/AST.hs` at `lara-syntax@0.9`, described here (the model was written
...
certificate premise reference may carry — Appendix G — and `@0.9` lowers
named atoms within the existing opaque certificate payload — Appendix H — so
none changes a `Lara.AST` type):
