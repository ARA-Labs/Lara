...
It is **not** parametricity, and the paper must not call it that: per #187's
acceptance criterion, "parametricity" is reserved for a relational
quantification over related backends, which is issue #215's future work. It is
...
That leaves a forward-looking obligation this phase does **not** discharge:
`Lara.Context.conclusionCache` and `Lara.Check.Program.conflictCache` compute
the same idea at two different times, and nothing yet proves they agree on a
linked program. F1 does not need it — `mem_conclusionCache` and
`conclusionCache_sound` are the inverses the saturation proofs consume — and it
is recorded as optional follow-up — **issue #226** — rather than left as an
...
`Semantics.ExtensionSemantics` is issue #216, deliberately out of scope: M2a's
...
**Blocked-by check (possible worlds).** #187 is blocked by the PW wrapper spike
only *if* possible-world observations become observable program behavior. They
have not. PW0 and PW-T6 have since landed (PRs #221, #223), and
...
## 10. Deferrals recorded here

- **Issue #219** — the five `DecidableEq` instances R-L3 needs
  (`Support.Question`, `Support.Rule`, `Attack.DefeatPolicy`,
  `Policy.RuleDecl`, `Policy.Policy`) are derived at the head of
  `lean/Lara/Context/Fragment.lean` rather than at their owning modules. That is
  a downstream home for core vocabulary and it makes visibility
  import-path-dependent; the move is mechanical and tracked.
- **Issue #226** — the cache-agreement bridge (§4): `conclusionCache` versus
  `Check.Program.conflictCache` on a linked program. Not needed by any phase
  that follows; tracked as optional cleanup, and the honest alternative is to
  drop the aspiration rather than prove it.
