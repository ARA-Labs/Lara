...
| **`rit`** (sibling project) | Lean kernel + sha256-pinned facts + AND/OR claim DAG | Monotonic proof-or-evidence attestation; **no defeasible defeat, no `gap`/`contested`/`defeated` distinction, no policy of inference schemes.** See `comparison-rit-lara.md` for the full sibling delta. |
