...
| **Pandžić** (2022, two papers) | Defeasible arguments as justification-logic terms with rebut/undercut/undermine | Pure logic, **no artifact application**; **factive** (keeps A1 globally — LARA has no warrant-level modality and confines factive logics behind the strict-backend interface); no policy / critical-question layer; no compilation-to-AF theorem; no evaluation. |
