...
- `lean/AxCheck.lean` already `#print axioms`-gates every headline row:
  `srcStatus_iff_checked` (:759), `srcIn_iff_checkedGrounded` (:757),
  `status_preservation` (:958), `grounded_stable` (:946),
  `contrary_claims_not_both_justified` (:997), `strict_step_sound` (:444),
  `ndBackend` (:446), `raBackend` (:472), `ndUses_eq_infer_deps` (:588).
...
| Term-level CQ templates and independently typed substitution (#217) | `Context.Holes.Template`, `AnswerTemplate`, `HasTemplate`, `FillingTyped`, `instantiate_hasSupport`, `eraseOpen_hasSupport` | `Lara/Context/Holes/Template.lean`, `Lara/Context/Holes/Syntax.lean` |
| Instantiated linking and contextual observation | `Context.Holes.HoleFragment`, `HoleContext`, `instantiateFragment`, `instantiated_support`, `link_attackComplete`, `link_checked`, `obsGen` | `Lara/Context/Holes.lean`, `Lara/Context/Holes/Link.lean` |
| Backend transport through term holes, fixing filling certificates too | `Context.Holes.FixesContext`, `RelFixesContext`, `backend_replacement_congruence_sem`, `backend_replacement_parametricity_sem` | `Lara/Context/Holes/Transport.lean` |
| Nested mandatory holes, actual substituted attack, errors and composition | `Examples.TermHoles.source_obligations_nonempty`, `filled_support`, `typed_link_checked`, `hole_observation`, `hole_wrong_answer`, `contexts_compose`, `certificate_context_not_fixed` | `Lara/Examples/TermHoles.lean` |
| Grounded contextual equivalence does not imply equivalence at every semantics in the open interface (#268) | `Examples.ContextualSeparation.grounded_ctxEquiv`, `singleton_not_ctxEquivSem`, `counterexample`, `grounded_does_not_imply_semantic`; witness uses the adequate family `singletonSem` | `Lara/Examples/ContextualSeparation.lean` |
