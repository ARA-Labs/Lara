...
producer remains untrusted. M5 verifies the separate structured surface-calculus model from an
already-constructed `Presentation.Program`/`Policy` through elaboration; it does not verify natural-
language generation or concrete parsing. Result 12 remains the structured codec round trip, while
the concrete `.lara` parser stays test-only.
...
structured S-expression codec round-trips the live presentation AST; it does
not validate the concrete `.lara` parser or elaboration. Result 14 begins with
that already-constructed AST and proves the supported surface-calculus and
elaboration contracts; it does not turn result 12 into a parser-correctness
theorem.
